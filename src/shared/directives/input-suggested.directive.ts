...
export class InputSuggestedDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.inputSuggested) {
      const start = this.inputSuggested.substring(0, this.inputSuggestedPart.length);
      const end = this.inputSuggested.substring(this.inputSuggestedPart.length, this.inputSuggested.length);
      const valueReplaced = `<strong>${start}</strong>${end}`;
      this.el.nativeElement.innerHTML = this.domSanitizer.sanitize(SecurityContext.HTML, valueReplaced);
    }
  }
}
